...
def select_data(dbname):
    ...
    cur.execute(f"""
                   SELECT `pdxset`.name,pdxset.id,pdxgoods.itemcode,sum(CASE WHEN pdxinv.type = 'PI' or pdxinv.type = 'PIOPEN'  THEN  pdxinv.qprice * pdxinv.qin else 0 END) as total_cost
                        ...
                        ,max(CASE WHEN pdxinv.type = 'PI' THEN  pdxinv.mprice  else 0 END ) * (sum(CASE WHEN pdxinv.type = 'SA'   THEN  pdxinv.qout  else 0 END) - sum(CASE WHEN pdxinv.type = 'SR'   THEN  pdxinv.qin else 0 END)) as sales_cost
    ...
    for setid in setlist:
        ...
        for d in data:
            if setid["id"] == d["id"]:
                ...
                sales_cost = float(sales_cost) + float(round(float(d["sales_cost"]),3))
