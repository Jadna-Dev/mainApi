...
def select_data(dbname):
    ...
    cur.execute(f"""
                   SELECT `pdxset`.name,pdxset.id,pdxgoods.itemcode,sum(CASE WHEN pdxinv.type = 'PI' or pdxinv.type = 'PIOPEN'  THEN  pdxinv.qprice * pdxinv.qin else 0 END) as total_cost
                        ...
                        ,pdxgoods.pdxcost2 * (sum(CASE WHEN pdxinv.type != 'jadjad' THEN pdxgoods.pdxcost * pdxinv.qin  else 0 END)-sum(CASE WHEN pdxinv.type != 'jadjad' THEN pdxgoods.pdxcost * pdxinv.qout  else 0 END)) AS stock_value
                                FROM pdxset
                                left JOIN `pdxgoods`
